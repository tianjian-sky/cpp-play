...
{
    E *pe = new E;
    // A 是虚基类，虚继承的目的是让某个类做出声明，承诺愿意共享它的基类。其中，这个被共享的基类就称为虚基类（Virtual Base Class），本例中的 A 就是一个虚基类。在这种机制下，不论虚基类在继承体系中出现了多少次，在派生类中都只包含一份虚基类的成员。
    A *pa = dynamic_cast<A *>(pe); // pa is actuallly point to E
    // B 不是虚基类，ambiguous conversion from derived class 'E' to base class 'B':
