...
int main()
{
    ...
    // const Item& it = static_cast<const Item&>(item);
    // const Item& it = reinterpret_cast<const Item&>(item);
    //  const Item& it = dynamic_cast<const Item&>(item);
    // it.set_value(0); this' argument to member function 'set_value' has type 'const Item', but function is not marked const
    it.print();
    item.print();
